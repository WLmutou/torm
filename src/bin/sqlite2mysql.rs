// torm实现将sqlite数据库中的数据迁移到mysql数据库中
//
// 通过 torm 的 Database API 同时连接 SQLite（源）与 MySQL（目标），
// 完成：
//   1. 从 SQLite sqlite_master + PRAGMA table_info 读取表结构与列定义
//   2. 将 SQLite 表结构映射为 MySQL 的 CREATE TABLE
//   3. 分批读取 SQLite 数据并写入 MySQL（支持事务）
//
// 用法：
...
//
// 位置参数 <sqlite_file> 为源 SQLite 数据库文件路径；MySQL 连接信息可通过
// 命令行覆盖，缺省时使用默认配置。
...
fn parse_args() -> Config {
    ...
    // 校验必需参数：SQLite 文件 + MySQL 连接（缺失时给出帮助）
...
async fn create_mysql_table(
    ...
    for col in columns {
        ...
        // 自增整数主键 → 列级 AUTO_INCREMENT PRIMARY KEY（MySQL 要求）
...
// ---------------------------------------------------------------------------
// 默认值规范化：把 SQLite 默认值转为 MySQL 可识别的形式
// ---------------------------------------------------------------------------
